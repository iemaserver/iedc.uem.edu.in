...
export function JournalFormDialog({ open, onOpenChange, journal, onSuccess }: JournalFormDialogProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      impactFactorDate: journal?.impactFactorDate ? new Date(journal.impactFactorDate).toISOString().split('T')[0] : "",
      publisher: journal?.publisher || "",
      status: journal?.status || "COMMUNICATED",
      statusDate: journal?.statusDate ? new Date(journal.statusDate).toISOString().split('T')[0] : "",
      paperLinkDOI: journal?.paperLinkDOI || "",
      registrationFees: journal?.registrationFees?.toString() || "",
      reimbursementStatus: journal?.reimbursementStatus || "",
      reimbursementDate: journal?.reimbursementDate ? new Date(journal.reimbursementDate).toISOString().split('T')[0] : "",
      isPublic: journal?.isPublic || false,
      authors: journal?.authors?.map((author: any, idx: number) => ({
  ...
  const getTeacherName = (teacherId: string) => {
    ...
    const author = journal?.authors?.find((a: any) => a.teacherId === teacherId);
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const payload: any = {
        ...
        typeOfJournal: data.typeOfJournal || undefined,
        indexOfJournal: data.indexOfJournal || undefined,
        ...
        reimbursementDate: data.reimbursementDate || undefined,
        isPublic: data.isPublic,
        authors: data.authors,
      };

      const url = isEdit ? `/api/teacher/journal/${journal.id}` : "/api/teacher/journal";
      ...
      alert(error.response?.data?.error || error.message || "Failed to save journal");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEdit ? "Edit" : "Add"} Journal Publication</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <FormField control={form.control} name="title" render={({ field }) => (
              <FormItem><FormLabel>Title *</FormLabel><FormControl><Input {...field} placeholder="Enter paper title" /></FormControl><FormMessage /></FormItem>
            )} />

            <FormField control={form.control} name="journalName" render={({ field }) => (
              <FormItem><FormLabel>Journal Name *</FormLabel><FormControl><Input {...field} placeholder="Enter journal name" /></FormControl><FormMessage /></FormItem>
            )} />

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField control={form.control} name="typeOfJournal" render={({ field }) => (
                <FormItem><FormLabel>Type of Journal</FormLabel><FormControl><Input {...field} placeholder="e.g., Peer-reviewed, Open Access" /></FormControl><FormMessage /></FormItem>
              )} />
              <FormField control={form.control} name="indexOfJournal" render={({ field }) => (
                <FormItem><FormLabel>Index of Journal</FormLabel><FormControl><Input {...field} placeholder="e.g., Scopus, SCI, Web of Science" /></FormControl><FormMessage /></FormItem>
              )} />
            </div>
            ...
            <FormField control={form.control} name="isPublic" render={({ field }) => (
              <FormItem className="flex items-center justify-between rounded-lg border p-3 shadow-sm">
                <div className="space-y-0.5">
                  ...
                  <div className="text-sm text-muted-foreground">Make this journal publicly visible</div>
                </div>
