...
const chartConfig = {
  count: { label: "Transactions", color: "hsl(280, 75%, 55%)" },
} satisfies ChartConfig;

export function GrowthGraph({
  transactions,
  title = "Transaction Growth",
  description = "Monthly cumulative growth of transactions",
  monthsToShow = 6,
}: GrowthGraphProps) {
    ...
    <Card className="flex flex-col bg-gradient-to-br from-fuchsia-50/50 via-white to-pink-50/50 dark:from-fuchsia-950/20 dark:via-background dark:to-pink-950/20 border-fuchsia-200/50 dark:border-fuchsia-800/30">
      ...
      <CardContent className="p-2">
        {totalTransactions === 0 ? (
          ...
          <ChartContainer
            config={chartConfig}
            className="w-full h-[200px] sm:h-[250px] lg:h-[160px] bg-gradient-to-br"
          >
            <AreaChart
              accessibilityLayer
              data={chartData}
