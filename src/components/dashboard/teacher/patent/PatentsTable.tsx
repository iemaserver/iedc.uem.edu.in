...
import { exportPatentsExcel } from "@/lib/csvExport";
...
export function PatentsTable() {
  ...
  const fetchPatentsData = async () => {
    setIsLoading(true);
    try {
      const result = await fetchPatents(filters);
  ...
  const handleMultiDelete = async () => {
    ...
    try {
      await deleteMultiplePatents(selectedIds);
      fetchPatentsData();
      setSelectedIds([]);
  ...
  const toggleSelectAll = () => {
    const allPageIds = patents.map((p) => p.id);
  ...
  return (
    <div className="w-full">
      <Card className="w-full bg-gradient-to-br from-green-50/50 via-white to-emerald-50/50 dark:from-green-950/20 dark:via-background dark:to-emerald-950/20 border-green-200/50 dark:border-green-800/30">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
            <div className="space-y-2">
              ...
              <CardDescription className="text-sm sm:text-base">
                Manage your patent applications and intellectual property
              </CardDescription>
            </div>
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <Button
                onClick={() => exportPatentsExcel(patents, "patents.xlsx")}
                variant="outline"
                ...
                <TableRow className="hover:bg-transparent">
                  <TableHead className="w-12">
                    <Checkbox
                      checked={
                        patents.length > 0 &&
                        patents.every((p) => selectedIds.includes(p.id))
                      }
                  ...
                  <TableHead className="min-w-[150px]">Application No</TableHead>
                  <TableHead className="min-w-[150px]">Patent Number</TableHead>
                  ...
                  <TableRow>
                    ...
                    >
                      {activeFilterCount > 0
                        ...
                        : "No patents found. Create your first one!"}
                    </TableCell>
                  </TableRow>
                ) : (
                  patents.map((patent) => {
                      ...
                      <TableRow key={patent.id} className="hover:bg-muted/50">
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          <Checkbox
                            ...
                            onCheckedChange={() => toggleSelectOne(patent.id)}
                            aria-label={`Select ${patent.title}`}
                          />
                        </TableCell>
                        <TableCell className="font-medium max-w-sm">
                          <div className="line-clamp-2" title={patent.title}>
                            {patent.title}
                          </div>
                        </TableCell>
                        <TableCell>
                          <TooltipProvider>
                            <div className="flex -space-x-2">
                              ...
                              {patent.inventors && patent.inventors.length > 3 && (
                        ...
                        <TableCell className="text-sm">
                          {patent.applicant || "N/A"}
                        </TableCell>
                        <TableCell className="font-mono text-xs">
                          {patent.applicationNo || "N/A"}
                        </TableCell>
                        <TableCell className="font-mono text-xs">
                          {patent.patentNumber || "N/A"}
                        </TableCell>
                        <TableCell className="text-sm">
                          {patent.filedAt ? formatDate(patent.filedAt) : "N/A"}
                        </TableCell>
                        <TableCell className="text-sm">
                          {patent.grantedAt
                            ? formatDate(patent.grantedAt)
                            : "N/A"}
                        </TableCell>

                        <TableCell>
                          <Badge
                            variant={patent.isPublic ? "default" : "secondary"}
                            className="text-xs"
                            style={
                              patent.isPublic
                                ? {
      ...
      <AlertDialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the patent entry.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={isMultiDeleteOpen} onOpenChange={setIsMultiDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Multiple Patents?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete {selectedIds.length} patent
