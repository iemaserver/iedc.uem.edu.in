...
const chartConfig = {
  count: { label: "Count" },
  public: { label: "Public", color: "hsl(45, 95%, 55%)" },
  private: { label: "Private", color: "hsl(35, 90%, 60%)" },
} satisfies ChartConfig;

export function StatusGraph({ patents, title = "Visibility Distribution", description = "Patents by visibility" }: StatusGraphProps) {
    ...
    <Card className="flex flex-col bg-gradient-to-br from-amber-50/50 via-white to-yellow-50/50 dark:from-amber-950/20 dark:via-background dark:to-yellow-950/20 border-amber-200/50 dark:border-amber-800/30">
      ...
      <CardContent className="p-2">
        {totalCount === 0 ? (
          ...
          <ChartContainer config={chartConfig} className="w-full h-[200px] sm:h-[250px] lg:h-[160px] bg-gradient-to-br from-amber-50/80 to-yellow-50/80 dark:from-amber-950/40 dark:to-yellow-950/40 rounded-lg">
      ...
      <CardFooter><div className="flex w-full items-center justify-center text-xs sm:text-sm text-muted-foreground">Public vs Private patents</div></CardFooter>
    </Card>
  );
}
