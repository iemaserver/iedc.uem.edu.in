...
import { fetchTeachers, createConference, updateConference } from "@/lib/api/teacherApi";
...
export function ConferenceFormDialog({ open, onOpenChange, conference, onSuccess }: ConferenceFormDialogProps) {
    ...
    if (open) {
      ...
      if (conference) {
        form.reset({
          ...
          conferenceStartDate: conference.conferenceStartDate ? new Date(conference.conferenceStartDate).toISOString().split('T')[0] : "",
          conferenceEndDate: conference.conferenceEndDate ? new Date(conference.conferenceEndDate).toISOString().split('T')[0] : "",
          status: conference.status || "COMMUNICATED",
          statusDate: conference.statusDate ? new Date(conference.statusDate).toISOString().split('T')[0] : "",
          ...
          reimbursementDate: conference.reimbursementDate ? new Date(conference.reimbursementDate).toISOString().split('T')[0] : "",
          isPublic: conference.isPublic ?? true,
        });
        setSelectedAuthors(conference.authors?.map((a: any) => a.teacher) || []);
      } else {
        form.reset({
          title: "",
          conferenceName: "",
  ...
  const filteredTeachers = teachers.filter(t => 
    ...
    (t.user.name.toLowerCase().includes(searchAuthor.toLowerCase()) || t.user.email.toLowerCase().includes(searchAuthor.toLowerCase()))
  );
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2 p-3 border rounded-md min-h-[50px]">
                {selectedAuthors.map((author) => (
                  <Badge key={author.id} variant="secondary" className="flex items-center gap-1">
                    {author.user.name}
                    <X className="h-3 w-3 cursor-pointer" onClick={() => setSelectedAuthors(prev => prev.filter(a => a.id !== author.id))} />
                  </Badge>
                ))}
              </div>
              <div className="relative">
                <div className="flex items-center gap-2">
                  ...
                  <Input placeholder="Search teachers..." value={searchAuthor} onChange={(e) => setSearchAuthor(e.target.value)} />
                </div>
                {searchAuthor && filteredTeachers.length > 0 && (
                  <div className="absolute z-50 w-full mt-1 bg-white border rounded-md shadow-lg max-h-48 overflow-y-auto">
                    {filteredTeachers.map((teacher) => (
                      <div key={teacher.id} className="px-3 py-2 hover:bg-gray-100 cursor-pointer" onClick={() => { setSelectedAuthors(prev => [...prev, teacher]); setSearchAuthor(""); }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="conferenceName" render={({ field }) => (
                <FormItem><FormLabel>Conference Name *</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )} />
            ...
            <div className="grid grid-cols-3 gap-4">
              <FormField control={form.control} name="mode" render={({ field }) => (
                <FormItem><FormLabel>Mode *</FormLabel><Select onValueChange={field.onChange} value={field.value}>
              ...
              <FormField control={form.control} name="typeOfConference" render={({ field }) => (
                <FormItem><FormLabel>Type</FormLabel><FormControl><Input placeholder="International/National" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
              <FormField control={form.control} name="indexOfConference" render={({ field }) => (
                <FormItem><FormLabel>Index</FormLabel><FormControl><Input placeholder="Scopus/IEEE" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="conferenceStartDate" render={({ field }) => (
                <FormItem><FormLabel>Start Date *</FormLabel><FormControl><Input type="date" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
              <FormField control={form.control} name="conferenceEndDate" render={({ field }) => (
            ...
            <FormField control={form.control} name="paperLinkDOI" render={({ field }) => (
              <FormItem><FormLabel>Paper Link / DOI</FormLabel><FormControl><Input placeholder="https://..." {...field} /></FormControl><FormMessage /></FormItem>
            )} />

            <div className="grid grid-cols-3 gap-4">
              <FormField control={form.control} name="registrationFees" render={({ field }) => (
                <FormItem><FormLabel>Registration Fees</FormLabel><FormControl><Input type="number" {...field} onChange={(e) => field.onChange(parseFloat(e.target.value) || 0)} /></FormControl><FormMessage /></FormItem>
              )} />
              <FormField control={form.control} name="reimbursementStatus" render={({ field }) => (
                <FormItem><FormLabel>Reimbursement Status</FormLabel><FormControl><Input placeholder="Approved/Pending" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
              <FormField control={form.control} name="reimbursementDate" render={({ field }) => (
                <FormItem><FormLabel>Reimbursement Date</FormLabel><FormControl><Input type="date" {...field} /></FormControl><FormMessage /></FormItem>
              )} />
            </div>
