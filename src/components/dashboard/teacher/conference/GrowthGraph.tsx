...
const chartConfig = {
  count: { label: "Conferences", color: "hsl(25, 95%, 53%)" },
} satisfies ChartConfig;

export function GrowthGraph({
  conferences,
  title = "Conferences Growth",
  description = "Monthly cumulative growth of conferences",
  monthsToShow = 6,
}: GrowthGraphProps) {
  const { chartData, trend } = useMemo(() => {
    ...
    return {
      ...
      trend: { value: Math.abs(Math.round(trendValue * 10) / 10), isPositive },
    };
        ...
        <div className="flex w-full items-start gap-2 text-xs sm:text-sm">
          <div className="grid gap-1 sm:gap-2">
            <div className="text-muted-foreground flex items-center gap-2 leading-none">
              Total: {totalConferences} conferences
            </div>
          </div>
        </div>
      </CardFooter>
