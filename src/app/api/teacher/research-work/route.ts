...
import { ResearchWorkType, PublicationStatus, UserType } from '@prisma/client';
import { z } from 'zod';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Ensure uploadedById matches the authenticated user
    body.uploadedById = userSession.id;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { page = 1, limit = 10, title, type } = parsedQuery.data;
    const pagination = getPaginationParams(page.toString(), limit.toString());

    // Build WHERE clause - only for the authenticated teacher
    const whereClause: any = {
      uploadedById: userSession.id,
    ...
    return NextResponse.json(createPaginatedResponse(researchWorks, totalResearchWorks, pagination));
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Only allow deletion of research works uploaded by the authenticated teacher
    const result = await prisma.researchWork.deleteMany({
      where: {
        id: {
          in: ids,
        },
        uploadedById: userSession.id, // Security: only delete own research works
      },
    });

    return NextResponse.json({
      message: `${result.count} research work(s) deleted successfully.`,
      deletedCount: result.count
    });
  } catch (error) {
    console.error("Error deleting research works:", error);
    return NextResponse.json(ApiErrors.INTERNAL_ERROR, { status: 500 });
  }
}