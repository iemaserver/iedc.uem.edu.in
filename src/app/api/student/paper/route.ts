...
export async function GET(req: NextRequest) {
  try {
    ...
    const [papers, total] = await Promise.all([
      prisma.researchPaper.findMany({
        ...
        include: {
          ...
          reviewedBy: {
            select: { id: true, userId: true, department: true, designation: true },
          },
        },
    ...
    return NextResponse.json({ data: papers, pagination: { total, page, limit, totalPages: Math.ceil(total / limit) } });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (session.user.role !== UserRole.STUDENT) {
      return NextResponse.json({ error: "Only students can create research papers" }, { status: 403 });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (memberIds && memberIds.length > 0) {
        await tx.researchPaperMember.createMany({
          data: memberIds.map(memberId => ({ researchPaperId: paper.id, memberId })),
        });
      }
      ...
      return tx.researchPaper.findUnique({
        ...
        include: {
          ...
          reviewedBy: { select: { id: true, userId: true } },
        },
      });
    });
