...
import { Loader2, Plus, X, Check, AlertCircle, User, Calendar, DollarSign, Activity } from "lucide-react";
...
export function AddTransactionDrawer({ open, onOpenChange, onSuccess }: Props) {
  ...
  const onSubmit = async (formData: FormSchema) => {
    try {
      const payload = {
        ...formData,
        impactFactorDate: formData.impactFactorDate?.toISOString() ?? null,
      ...
      const response = await axios.post("/api/teacher/transactions", payload);
      ...
      <DrawerContent className="w-full h-[90vh] sm:h-[85vh] mt-auto rounded-t-2xl overflow-hidden bg-slate-50 dark:bg-slate-900 flex flex-col">
        <DrawerHeader className="border-b bg-gradient-to-r from-slate-800 to-slate-900 dark:from-slate-800 dark:to-slate-900 flex-shrink-0">
          ...
          <p className="text-sm text-slate-300 text-center">
            Fill in the details to register a new research transaction
          </p>
        </DrawerHeader>
        ...
        <div className="flex flex-col min-h-0 flex-1">
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-3 sm:px-4 md:px-6 py-3 sm:py-4 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 dark:scrollbar-thumb-slate-600 dark:scrollbar-track-slate-800 scroll-smooth">
            <Form {...form}>
              <form id="transaction-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6 pb-8 sm:pb-12">
                ...
                <Card className="border-2 border-blue-200 dark:border-blue-800 shadow-sm hover:shadow-md transition-shadow bg-white dark:bg-slate-800">
                  ...
                  <CardContent className="p-6 space-y-4">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-sm font-medium">Transaction Name *</FormLabel>
                          <FormControl>
                            <Input
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel className="text-sm font-medium">Type of Transaction</FormLabel>
                            <FormControl>
                              <Input
                        ...
                        <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                          <div className="space-y-0.5">
                            ...
                            <div className="text-sm text-gray-600">
                              Allow others to view this transaction record
                            </div>
                          </div>
                          <FormControl>
                            <Switch
